'''
modified LMMD loss for multi-label classification
'''
...
class LMMD_loss(nn.Module):
    ...
    def cal_weight(self, s_label, t_label, batch_size=32, class_num=31):
        ...
        s_sum = np.sum(s_vec_label, axis=0).reshape(1, class_num).astype('int')
        s_sum[s_sum == 0] = 100
        ...
        s1_sum = np.sum(s_vec_label, axis=0).reshape(-1)
        t1_sum = np.sum(t_vec_label, axis=0).reshape(-1)
        index = [ i for i in range(class_num) if s1_sum[i]>0 and t1_sum[i]>0]
